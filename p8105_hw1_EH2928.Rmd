---
title: "assignment_1_rmarkdown"
author: "Emil Hafeez (eh2928)"
date: "9/14/2020"
output: github_document
---
#Starting Small

This is an R Markdown file for P8105 Homework 1. I focus on meaningful naming schemas, readable code, clearly-written explanatory text, and parsimony. 

##Library Loaders Section
```{r}
library(tidyverse)
```


#Problem 1

We create a data frame which contains a random sample of size 10 from a normal distribution, a logical vector indicating whether elements of the sample are greater than 0, a character vector of length 10, and a factor vector of length 10 with 3 different factor levels. 

```{r}
#Load and use tibble to create aforementioned data frame
practice_df = tibble(
    vec_rnorm_numeric = rnorm(10),
    vec_greater_than_zero_logical = (vec_rnorm_numeric > 0), 
    vec_building_characters = c("Will", "I", "always", "get", "a", "headache", "from", "thinking", "too", "hard"),
    vec_factor_temperature = factor(c("Fahrenheit", "Fahrenheit", "Celsius", "Fahrenheit", "Kelvin", "Fahrenheit", "Celsius", "Kelvin", "Kelvin", "Fahrenheit"))
)

#Take the mean after each variable.
#I learn, after 2 hours, you need to use $ to select the variable from the dataframe! Duh.
#Taking the mean() of all variables creates a N/A error, as anticipated, and so we mean() each individually for illustrative purposes.
mean(practice_df$vec_rnorm_numeric)
mean(practice_df$vec_greater_than_zero_logical) 
mean(practice_df$vec_building_characters)
mean(practice_df$vec_factor_temperature)
##The numeric and logical vectors work for this, and does not for string (characters and factors) variables.
```
##Create code chunk for converting variable types

```{r echo = true, results = 'hide'}
as.numeric(practice_df$vec_greater_than_zero_logical, practice_df$vec_building_characters, practice_df$vec_factor_temperature)


```

